...
define_responses("Responses") do |responses|
  ...
  response_for responses, :name => :show_best_order,:type => 0x1a,
    :body => [[:order_type, :uint8], [:stock_id, :uint32],
              [:volume, :uint32], [:price, :uint32]]

    response_for responses, :name => :show_no_best_order, :type => 0x25,
    :body => [[:order_type, :order_type],[:stock_id, :uint32]]

  response_for responses, :name => :list_of_stocks, :type => 0x1e,
    :body => [[:stocks, :list_of_stocks]]
  response_for responses, :name => :list_of_orders, :type => 0x20,
    :body => [[:orders, :list_of_orders]]
  response_for responses, :name => :register_successful, :type => 0x1,
    :body => [[:user_id, :uint32]]

  response_for responses, :name => :stock_info, :type => 0x22,
    :body => [[:stock_id, :uint32], [:best_buy_order, :best_order],
              [:best_sell_order, :best_order], [:last_transaction, :last_transaction]]
